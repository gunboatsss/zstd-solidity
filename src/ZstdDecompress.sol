...
library ZstdDecompress {
    ...
    function decompress(bytes memory srcData) internal pure returns (bytes memory dstData) {
        ...
        function readLE32(p) -> v {
            v := or(or(or(read8(p), shl(8, read8(add(p, 1)))),
                shl(16, read8(add(p, 2)))), shl(24, read8(add(p, 3))))
        }
        ...
        function highbit32(x) -> r {
            ...
            if iszero(x) { r := 0
                leave }
            r := 0
            if gt(x, 0xFFFF) { x := shr(16, x)
                r := add(r, 16) }
            if gt(x, 0xFF)   { x := shr(8, x)
                r := add(r, 8)  }
            if gt(x, 0xF)    { x := shr(4, x)
                r := add(r, 4)  }
            if gt(x, 0x3)    { x := shr(2, x)
                r := add(r, 2)  }
        ...
        function tzeros(x) -> r {
            if iszero(x) { r := 32
                leave }
            r := 0
            if iszero(and(x, 0xFFFF)) { x := shr(16, x)
                r := add(r, 16) }
            if iszero(and(x, 0xFF))   { x := shr(8, x)
                r := add(r, 8)  }
            if iszero(and(x, 0xF))    { x := shr(4, x)
                r := add(r, 4)  }
            if iszero(and(x, 0x3))    { x := shr(2, x)
                r := add(r, 2)  }
        ...
        function llBase(sym) -> v {
            ...
            if eq(sym, 26) { v := 0x80
                leave }
            if eq(sym, 27) { v := 0x100
                leave }
            if eq(sym, 28) { v := 0x200
                leave }
            if eq(sym, 29) { v := 0x400
                leave }
            if eq(sym, 30) { v := 0x800
                leave }
            if eq(sym, 31) { v := 0x1000
                leave }
            if eq(sym, 32) { v := 0x2000
                leave }
            if eq(sym, 33) { v := 0x4000
                leave }
            if eq(sym, 34) { v := 0x8000
                leave }
            if eq(sym, 35) { v := 0x10000
                leave }
        }
        ...
        function mlBase(sym) -> v {
            ...
            if eq(sym, 43) { v := 0x83
                leave }
            if eq(sym, 44) { v := 0x103
                leave }
            if eq(sym, 45) { v := 0x203
                leave }
            if eq(sym, 46) { v := 0x403
                leave }
            if eq(sym, 47) { v := 0x803
                leave }
            if eq(sym, 48) { v := 0x1003
                leave }
            if eq(sym, 49) { v := 0x2003
                leave }
            if eq(sym, 50) { v := 0x4003
                leave }
            if eq(sym, 51) { v := 0x8003
                leave }
            if eq(sym, 52) { v := 0x10003
                leave }
        }
...
// ================================================================
        ...
        function seqNbAddBits(tabType, sym) -> b {
            if eq(tabType, 0) { b := llBits(sym)
                leave }
            if eq(tabType, 1) { b := ofBits(sym)
                leave }
            b := mlBits(sym)
        }
        function seqBaseValue(tabType, sym) -> v {
            if eq(tabType, 0) { v := llBase(sym)
                leave }
            if eq(tabType, 1) { v := ofBase(sym)
                leave }
            v := mlBase(sym)
        }
        ...
        function buildSeqTable(sp, se, typ, maxS, maxL, dt, wk, tabType) -> np, tlog {
            np := sp
            if eq(typ, 0) {
                if eq(tabType, 0) { tlog := 6
                    writeLLTable(dt) }
                if eq(tabType, 1) { tlog := 5
                    writeOFTable(dt) }
                if eq(tabType, 2) { tlog := 6
                    writeMLTable(dt) }
                leave
            }
            if eq(typ, 1) {
        ...
        function execSequences(
            dstPos, dstEnd, litPtr, litSize,
            seqSrc, seqSrcSize, nbSeq,
            llTab, llLog, ofTab, ofLog, mlTab, mlLog,
            rep0, rep1, rep2
        ) -> nd, r0, r1, r2 {
            ...
            for { } 1 { } {
                ...
                if eq(blockType, 2) {
                    ...
                    if nbSeq {
                        ...
                        dstPos, rep0, rep1, rep2 := execSequences(
                            dstPos, add(dstBase, dstCap),
                            litPtr, litSize,
                            blkPtr, sub(blockEnd, blkPtr), nbSeq,
                            llTab, llLog, ofTab, ofLog, mlTab, mlLog,
                            rep0, rep1, rep2
                        )
                    }
