// SPDX-License-Identifier: BSD-3-Clause
pragma solidity ^0.8.20;
import {Test} from "forge-std/Test.sol";
import {ZstdDecompress} from "../src/ZstdDecompress.sol";

contract HufTest is Test {
    function test_huff_decompress() public {
        bytes memory c = hex"";
        bytes memory r = ZstdDecompress.decompress(c);
        assertEq(r.length, 10890, "length");
        string memory expected = "Line 0: The quick brown fox jumps over the lazy dog!\n";
        for (uint i = 0; i < bytes(expected).length; i++) {
            assertEq(r[i], bytes(expected)[i], "first line");
        }
    }
}
